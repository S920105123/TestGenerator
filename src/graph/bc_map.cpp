...
void BC_map::random_walk(vector<int> &result) {
    /* Randomly select a vertex to start with, then randomly walk until no outgoing edges */
    // cout << "I enter random walk.\n" << std::flush;
    static const int MIN_LENGTH = 10;    // At least how many vertices on the path
    static const int MAX_LENGTH = 10000; // Stop if path length exceed this value

    static std::default_random_engine generator(time(NULL));
    static std::uniform_int_distribution<int> distribution(0, G.size() - 1);
    // cout << "I call some STL functions, because I am lazy.\n" << std::flush;

    result.clear();
    while (result.size() < MIN_LENGTH) {
        int cur = distribution(generator);
        // cout << "And then I throw a dice, get " << cur << ".\n" << std::flush;
        result.clear();
        result.push_back(cur);
        while (result.size() < MAX_LENGTH && !G[cur].empty()) {
            static std::uniform_int_distribution<int> edge_distr(0, G[cur].size() - 1);
            cur = G[cur][edge_distr(generator)]->to;
            // cout << cur << " " << get_graph_id(cur) << "\n" << std::flush;
            if (graph->nodes[get_graph_id(cur)].type == CLOCK) {
                result.clear();
                break;
            }
            result.push_back(cur);
