...
void Graph::print_through(ostream &fout, int bc_id) {
	Transition_Type type = bc_map->get_graph_id_type( bc_id );
	int gid = bc_map->get_graph_id(bc_id);

	if (type == FALL) fout << "-fall_";
	else fout << "-rise_";

	if (bc_map->G[bc_id].empty()) fout << "to ";
	else if (bc_map->Gr[bc_id].empty()) fout << "from ";
	else fout << "through ";

	const string &name = this->nodes[gid].name;
	for (char c : name) {
		if (c == ':') fout << "/";
		else fout << c;
	}
...
void Graph::gen_test(const string &filename){
	vector<int> path, through;
	vector<bool> fixed;
	ofstream fout;
	fout.open(filename);
	this->init_graph();

	const int EXPECTED_FIX_NUM = 7;
	const int ITERATION = 3500;

	for (int t = 0; t < ITERATION; t++) {
		static std::default_random_engine generator(time(NULL));
	    static std::uniform_real_distribution<double> pdf(0.0, 1.0);

		bc_map->random_walk(path);
		// cout << "Path:"; for (int x : path) print_through(cout, x), cout << '\n';
		double prob = (double)EXPECTED_FIX_NUM / path.size();
		fixed.resize(path.size());
		through.clear();
		for (int i = 0; i < (int)path.size(); i++) {
			double tmp = pdf(generator);
			// cout << "Generate " << tmp << "\n";
			if (tmp < prob) {
				through.push_back(path[i]);
				fixed[i] = true;
			}
			else fixed[i] = false;
		}
		// cout << "Fix:"; for (int x : through) print_through(cout, x), cout << '\n';
		...
		for (int i = 0; i < (int)path.size(); i++) {
			if (!fixed[i]) {
				int j;
				fout << "report_timing ";
				for (j = 0; j < before; j++) print_through(fout, through[j]);
				print_through(fout, path[i]);
				for (; j < (int)through.size(); j++) print_through(fout, through[j]);
				fout << "\n";
			}
			else before++;
		}
